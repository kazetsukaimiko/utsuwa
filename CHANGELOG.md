...
- **MCP session resume and idle**: pass `resumeId` (`GROK_SESSION_ID`) on `set_session` to reconnect the same avatar. Unclaimed sessions expire in 90s; claimed in 10 minutes. Character config is stored under `~/.config/utsuwa/mcp-server/sessions/`.
